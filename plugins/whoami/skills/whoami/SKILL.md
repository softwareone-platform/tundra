...
Confirm the repositories, the language, and the sources in one AskUserQuestion call. When the scope holds more than three repositories, say that each repository with the person's changes adds time and tokens, and that one without them is skipped quickly. Say plainly what each source sends to the model:
...
Read up to 100 changes in each repository, unless the person asked for a different number: the 60 newest, and 40 spread evenly across everything older, taking every so many changes in commit order rather than one per stretch of calendar, so a period of heavy work gets its share of the sample. The recent past is read densely because it shows how the person works now. The older history is read thinly, but it is read, because step 5 needs instances from before AI shows up, and a gap that stops appearing is worth reporting too. With 100 or fewer changes in a repository, read them all. Read them a commit at a time.
