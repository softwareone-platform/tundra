...
- The **timeline** is drawn under the scope line, above the conclusion, so the reader sees what the conclusion rests on before reading it. Every date is written `YYYY-MM-DD` and is one you measured, never an estimate. `today` is the date you write the report, where the timeline ends, so a repository the person has stopped working in reads as stopped. A repository's `from` and `to` are the dates of the oldest and newest of its changes you read. A repository in the scope that holds none of the person's changes is listed by its `name` alone and drawn as not read, with no `ai_from`, because AI showing up there says nothing about them. A repository's `recent_from` is the date of the oldest of its newest changes, the ones you read closely, and its changes before that date are drawn as the thinner sample. Each repository has its own, because each is sampled on its own. Leave it out for a repository whose changes you read every one of. The renderer rejects a `recent_from` outside the repository's `from` and `to`, and one given for the whole timeline. Each row is drawn solid where every change was read, hatched where they were sampled, and as an empty box where nothing was read. `ai_from` is the date of the earliest evidence that AI took part, left out when there is none. `prompts` spans the first and last prompt you read, left out when you read none. The timeline is optional, and the Markdown summary leaves it to the scope line.
...
- `axes` usually holds one axis, and a second only when its mechanism differs from the first. An axis's `patterns` may mix groups. Each side of the axis lists the ones from its own group, strengths on the side it gets right and gaps on the side it misses. A style belongs to neither side, so the axis does not show it, and it appears in its own table.
