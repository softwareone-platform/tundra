...
- **The report's language can be asked for in words after the paths**, such as "in Traditional Chinese", and otherwise follows the language of the session. People often steer Claude in English and read a long report more easily in their own language. Words rather than a flag, because a skill's arguments reach the model as text anyway, and a path is told from such words by whether it exists.
